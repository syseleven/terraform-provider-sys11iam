...
resource "ncs_organization_contact" "testorganization_contact" {
  count = data.ncs_organization.testorg.is_active ? 1 : 0
  first_name = "Test"
  last_name = "Contact"
  notes = "test notes"
...
resource "ncs_organization_team" "testorganization_team" {
  count = data.ncs_organization.testorg.is_active ? 1 : 0
  name = "testteam"
...
resource "ncs_project_team" "test_project_team" {
  ...
  organization_id = data.ncs_organization.testorg.id
  project_id = ncs_project.test_project[0].id
  team_id = ncs_organization_team.testorganization_team[0].id
  editable_permissions = ["can_become_administrator_in_project"]
}

# Create an NCS organization team membership
resource "ncs_organization_team_membership" "testorganization_team_membership" {
  count = data.ncs_organization.testorg.is_active ? 1 : 0
...
The following arguments are supported for the data source "ncs_organization":
...
The following arguments are supported for the resource "ncs_organization_serviceaccount":

* **`name`** - The name of the service account.
* **`description`** - The description of the service account.
* **`organization_id`** - The UUID of the organization.

The following arguments are supported for the resource "ncs_organization_contact":
...
The following arguments are supported for the resource "ncs_organization_team":
...
The following arguments are supported for the resource "ncs_project_team":
...
The following arguments are supported for the resource "ncs_organization_team_membership":

* **`id`** - The UUID of the regular user or service account to add as a member.
* **`organization_id`** - The UUID of the organization.
* **`team_id`** - The UUID of the organization team.

The following arguments are supported for the resource "ncs_project_s3user":
